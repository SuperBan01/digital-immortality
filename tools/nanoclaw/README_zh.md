...
NanoClaw 支持 [MiniMax](https://platform.minimaxi.com)（稀宇科技）API，提供自动格式转换。在 `.env` 中配置：

```bash
# MiniMax API 密钥
MINIMAX_API_KEY=your-minimax-api-key

# 可选：指定模型（默认为 abab6.5s-chat）
MINIMAX_MODEL=abab6.5s-chat
```

当设置 `MINIMAX_API_KEY` 后，凭证代理会自动：
1. 拦截来自 Claude Agent SDK 的 Anthropic 格式请求
2. 将其转换为 MiniMax 的 OpenAI 兼容格式
3. 发送至 MiniMax API (`https://api.minimax.chat/v1/text/chatcompletion_v2`)
4. 将响应转换回 Anthropic 格式

这使得在使用 MiniMax 模型时，仍能保持与 Claude Agent SDK 的完全兼容。

### 控制台渠道（测试用）

NanoClaw 内置控制台渠道，无需外部消息平台即可测试。发送消息方式：
