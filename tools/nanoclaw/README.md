...
NanoClaw supports [MiniMax](https://platform.minimaxi.com) (稀宇科技) API with automatic format translation. Configure in your `.env`:

```bash
# MiniMax API Key
MINIMAX_API_KEY=your-minimax-api-key

# Optional: specify model (defaults to abab6.5s-chat)
MINIMAX_MODEL=abab6.5s-chat
```

When `MINIMAX_API_KEY` is set, the credential proxy automatically:
1. Intercepts Anthropic-format requests from the Claude Agent SDK
2. Translates them to MiniMax's OpenAI-compatible format
3. Sends to MiniMax API (`https://api.minimax.chat/v1/text/chatcompletion_v2`)
4. Translates responses back to Anthropic format

This allows full compatibility with the Claude Agent SDK while using MiniMax models.

### Console Channel (Testing)

NanoClaw includes a built-in console channel for testing without external messaging platforms. Messages are sent via a file:

```bash
echo "your message" > /tmp/nanoclaw-console-input
```

The agent response appears in the terminal. This is useful for:
- Testing API configuration
- Debugging agent behavior
- Quick iteration without platform setup
